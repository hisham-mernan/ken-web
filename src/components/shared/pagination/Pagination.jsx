...
const Pagination = ({
  ...
  const onNext = () => {
    if (totalCount > currentPage) {
      onPageChange(currentPage + 1 < totalCount ? currentPage + 1 : totalCount);
    }
  };
  ...
  return (
    <div
      className={`flex_center gap-2 sm:gap-6  bg-white py-2  px-4 sm:px-6 rounded-[10px] border border-font-light  mx-auto ${className}`}
    >
      {" "}
      <div
        className={`flex_center py-2 px-3 rounded-sm border border-font-light ${
          currentPage > 1 ? " cursor-pointer" : "opacity-20 cursor-default"
        }`}
        onClick={onPrevious}
      >
        <span
        ...
        >
          <TrailingIcon fill="var(--color-primary-dark)" />
        </span>{" "}
        <span className="text-primary-dark text-sm hidden sm:flex">
          {t("back")}
        </span>
      </div>
        ...
        {paginationRange?.map((pageNumber, index) => {
          if (pageNumber === DOTS) {
            return (
              <div
                key={index}
                className="flex_center w-8 h-8  sm:h-9  border border-font-light  rounded-lg dots"
              >
                <svg
                  width="13"
                  height="4"
          ...
          return (
            <span
              key={index}
              role="button"
              className={`flex_center w-8 h-8  sm:h-9  border border-font-light hover:bg-primary-2 hover:text-[#FDF9FF] transition-all ease-in-out duration-300   rounded-lg   cursor-pointer font-inter text-base font-medium ${
                pageNumber === currentPage
                  ? "bg-primary-2 text-[#FDF9FF] font-bold"
                  : "text-primary-dark"
              }`}
      ...
      >
        ...
        <span
          className={` ${currentPage === lastPage ? "opacity-20" : ""} ${
            currentLanguageCode === "en" ? "" : "rotate-180"
          }`}
        >
          <TrailingIcon fill="var(--color-primary-dark)" />
